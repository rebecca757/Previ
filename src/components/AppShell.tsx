...
function ProfileSwitcher() {
  const { ownId, activeId, activeProfile, managed, profileType, setActive } = useActiveProfile();
  ...
  const currentInitial = activeProfile?.full_name?.[0]?.toUpperCase() || "?";
          ...
          <>
            ...
            {managed.map((m) => (
              <DropdownMenuItem key={m.id} onClick={() => setActive(m.id)}>
                ...
                <span className="flex-1 text-sm">{m.full_name || "Account collegato"}</span>
                {profileType === "family_link" && activeId === m.id && (
...
function ManagingBanner() {
  const { isManaging, activeProfile } = useActiveProfile();
  if (!isManaging) return null;

  const name = activeProfile?.full_name;
  const a = age(activeProfile?.date_of_birth);
  ...
  return (
    ...
    >
      ...
      <span>
        Stai visualizzando l'account di <strong>{name}</strong>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-64 border-r bg-card flex-col">
        <div className="p-6">
          <Link to="/dashboard" className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-primary grid place-items-center text-primary-foreground font-bold">
              P
            </div>
